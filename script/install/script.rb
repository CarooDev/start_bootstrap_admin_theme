...
FONT_AWESOME_RELATIVE_PATH = 'vendor/fontawesome-free/css'
START_BOOTSTRAP_FONTAWESOME_PATH = START_BOOTSTRAP_PATH.join(FONT_AWESOME_RELATIVE_PATH)
...
def font_awesome_stylesheets_target_path
  @font_awesome_stylesheets_target_path ||= target_assets_path('stylesheets').join(FONT_AWESOME_RELATIVE_PATH)
end
...
def cp_font_awesome_css_files
  FileUtils.mkdir_p(font_awesome_stylesheets_target_path)

  START_BOOTSTRAP_FONTAWESOME_PATH.glob('*.css').each do |css_pathname|
    FileUtils.cp(css_pathname, font_awesome_stylesheets_target_path.join("#{css_pathname.basename}.erb"))
  end
end
...
def update_paths_in_font_awesome_css_files
  font_awesome_stylesheets_target_path.glob('*.css.erb').each do |pathname|
    updated_contents = pathname.read
...
update_paths_in_font_awesome_css_files
